...
#
# Real bug found in this script itself: the original heredoc that
# created it had blank lines inserted between each backslash-continued
# line, silently breaking every continuation -- bash only joins a
# continuation to the IMMEDIATELY following line, an intervening
# blank line ends the command early. Confirmed via hexdump that the
# backslash+newline bytes themselves were correct; the blank line
# right after was the actual bug. Rewritten as one single-line
# docker run invocation to make this class of bug impossible here.
set -euo pipefail
cd "$(dirname "$0")"
docker run --rm -v "$(pwd)/../..":/build -w /build/agent/rust -e RUSTFLAGS="-L /usr/lib -l static=zstd" rust:alpine sh -c "apk add --no-cache musl-dev linux-headers libbpf-dev elfutils-dev zlib-dev zlib-static libelf-static zstd-static clang llvm make pkgconfig && cargo build --release --target x86_64-unknown-linux-musl"
