# STATUS: 100% — DuckDB incident storage
...
class RawAlert:
    ...
    @classmethod
    def create(cls, source, severity, pid=0, host="", comm="",
               reason="", event_type="", raw_json="{}"):
        return cls(
            alert_id=str(uuid.uuid4()), source=source, severity=severity,
            ts=now_utc(), pid=pid, host=host, comm=comm,
            reason=reason, event_type=event_type, raw_json=raw_json,
        )
...
class IncidentStore:
    ...
    def _init_schema(self):
        with self._conn() as con:
            ...
            con.execute("""CREATE TABLE IF NOT EXISTS alerts (
                ...
                host VARCHAR NOT NULL DEFAULT '',
                comm VARCHAR NOT NULL DEFAULT '',
                reason VARCHAR NOT NULL DEFAULT '',
                event_type VARCHAR NOT NULL DEFAULT '',
                raw_json JSON NOT NULL DEFAULT '{}')""")

    def save_incident(self, i):
        params = [
            i.incident_id, i.created_at, i.updated_at, i.host,
            i.severity.value, i.confidence, i.window_type,
            i.tactic_id, i.tactic_name, i.technique_id, i.technique_name,
            i.alert_count, json.dumps(i.alert_ids),
        ...
        with self._conn() as con:
            con.execute("""INSERT INTO incidents VALUES
                ...
                closed=excluded.closed""", params)

    def save_alert(self, a, incident_id):
        with self._conn() as con:
            con.execute("""INSERT INTO alerts VALUES
                (?,?,?,?,?,?,?,?,?,?,?)
                ON CONFLICT (alert_id) DO NOTHING""", [
                a.alert_id, incident_id, a.source.value, a.severity.value,
                a.ts, a.pid, a.host, a.comm, a.reason, a.event_type, a.raw_json,
            ])

    def close_incident(self, incident_id):
        with self._conn() as con:
            con.execute(
                "UPDATE incidents SET closed=TRUE, updated_at=? WHERE incident_id=?",
                [now_utc(), incident_id])
    ...
    def get_incidents_in_range(self, start, end, host=""):
        ...
        if host: q += " AND host=?"; p.append(host)
        with self._conn() as con:
            rows = con.execute(q + " ORDER BY created_at DESC", p).fetchall()
        return [self._row(r) for r in rows if r]
    ...
    def _row(r):
        return Incident(
            incident_id=r[0], created_at=r[1], updated_at=r[2], host=r[3],
            severity=Severity(r[4]), confidence=r[5], window_type=r[6],
            tactic_id=r[7], tactic_name=r[8], technique_id=r[9],
            technique_name=r[10], alert_count=r[11],
            alert_ids=json.loads(r[12]) if isinstance(r[12], str) else r[12],
            sources=json.loads(r[13]) if isinstance(r[13], str) else r[13],
            summary=r[14], closed=r[15],
        )
